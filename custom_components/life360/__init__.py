...
async def async_setup_entry(hass: HomeAssistant, entry: L360ConfigEntry) -> bool:
    ...
    async def async_process_data(forward: bool = False) -> None:
        ...
        for mid in mids - set(mem_coordinator):
            ...
            # Do not block entry setup on an initial member refresh.  These
            # requests sit in long retry loops when Life360 is returning
            # transient 403s, causing Home Assistant to cancel setup entirely.
